...
public class ProdutosController : ControllerBase
{
    ...
    [Authorize(Policy = "UserOnly")] // apenas usuários com a role User podem acessar este endpoint
    ...
    public async Task<ActionResult<IEnumerable<Produto>>> Get()
    {
        ...
        {
            return BadRequest(); // 400 Bad Request se ocorrer algum erro durante a execução
        }
    }
    ...
    public async Task<ActionResult<Produto>> GetId(int? id)
    {
        ...
        if (id == null || id <= 0) return BadRequest(); // 400 Bad Request se o ID for nulo ou menor ou igual a zero) 
    ...
    public async Task<ActionResult<ProdutoDTO>> Post(ProdutoDTO p)
    {
        if (p == null) return BadRequest(); // 400 Bad Request se não houver o produto

        var produto = _mapper.Map<Produto>(p); // mapeia o ProdutoDTO para a entidade Produto
        await _uof.CommitAsync();

        var novoProdutoDto = _mapper.Map<ProdutoDTO>(produto); // mapeia o produto recém-criado de volta para ProdutoDTO
    ...
    public async Task<ActionResult<ProdutoDTO>> Put(int id, ProdutoDTO p)
    {
    ...
    public async Task<ActionResult<ProdutoDTO>> Delete(int id)
    {
        ...
        var produtoDeletadoDto = _mapper.Map<ProdutoDTO>(produtoDeletado); // mapeia o produto deletado para ProdutoDTO
